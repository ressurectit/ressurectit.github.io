...
Basic simple usage of **new Matrix** *Grid* with data loaded asynchronously from *REST* service, simple columns and row selection enabled.
...
- `GridOptions` initialized with
    ...
    - row selector type set to `BasicRowSelectorComponent`
    - row selector options set
        - `getRowData` sets how to obtain selected data
        - `getRowId` sets how to obtain selected ids (unique)
- implemented logic for handling selection of all data, checking if any selected
- html `<div>` is top level element, can be any element
    ...
    - it contains templates for *columns*
        ...
        - column that handles and display selection of rows

@SAMPLE#row-selection-grid&grid/RowSelectionComponent@