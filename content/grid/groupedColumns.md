...
Grouped columns usage of **new Matrix** *Grid* with data loaded asynchronously from *REST* service and grouped columns.
...
- html `<div>` is top level element, can be any element
    ...
    - it contains two `headerRowContainerTemplate` templates, one for grouped headers and second for actual columns
        - grouped columns are styled using `grid-column` and *span* to look as they should

@SAMPLE#grouped-column-grid&grid/GroupedColumnsComponent@